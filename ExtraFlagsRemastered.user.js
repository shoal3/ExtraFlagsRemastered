...
// @name        Extra Flags Remastered
// @namespace   https://github.com/shoal3/ExtraFlagsRemastered
// @description Alternative userscript and repository for Extra Flags.
...
// @include     http*://boards.4chan.org/sp/catalog
...
// @include     http*://boards.4channel.org/pol/catalog
...
var setup = {
    ...
    html: function () {
        ...
        var htmlFixedStart = '<div style="font-size:200%;font-weight:bold;color:#ffffff">Extra Flags Setup</div><br/>';
        ...
        var filterRadio = '<br/><br/><form id="filterRadio">' +
            '<input type="radio" name="filterRadio" id="filterRadioall" style="display: inline !important;" value="all"><label style="color:#ffffff"> Show country + All selected regions.</label>' +
            '<br/><input type="radio" name="filterRadio" id="filterRadiofirst" style="display: inline !important;" value="first"><label style="color:#ffffff"> Show country + Only the first selected region.</label>' +
            '</form>';
        var whiteRegional = '<div style="color:#ffffff;font-size:85%;image-rendering:pixelated">Flags selected:  '
        ...
        if (regions.length > 1) {
            ...
            for (var i = 1; i < regions.length; i++) {
                path += "/" + regions[i];
                whiteRegional += "<img src=\"" + path + ".png\"" + " title=\"" + regions[i] + "\"> ";
            }
            whiteRegional += "<br/>";
            return htmlFixedStart + '<div style="color:#ffffff;font-weight:bold;">Region: <br/><select id="' + shortId + 'countrySelect">' +
                '</select></div><br/>' + htmlBackNextButtons +
                '<br/>' + htmlSaveButton + '</div>' + whiteRegional + filterRadio;
        }

        if (regions.length == 1) {
            var whiteRegional = "<br/>";
            return htmlFixedStart + '<div style="color:#ffffff;font-weight:bold;">Region: <br/><select id="' + shortId + 'countrySelect">' +
                '</select></div><br/>' + htmlBackNextButtons +
                whiteRegional + filterRadio;
       }

        return htmlFixedStart + '<div style="color:#ffffff;font-weight:bold;">Country: <br/><select id="' + shortId + 'countrySelect">' +
            '</select></div><br/>' + htmlBackNextButtons + '<br/>' + filterRadio;
    ...
    show: function () {
        ...
        GM_addStyle('\
            #' + setup.id + ' { position:fixed;z-index:10001;top:55px;right:40px;padding:20px 30px;background-color:#1d1f21;width:275px;border:0px solid #505050;border-radius:9px;box-shadow: 0px 1px 10px 1px #808080; }\
            #' + setup.id + ' * { color:black;text-align:left;line-height:normal;font-size:12px }\
            #' + setup.id + ' div { text-align:center;font-size:14px }'
        );
        ...
        if (postedRegions.length > 0 && !(currentFlag === undefined)) {
            ...
            for (var i = 0; i < postedRegions.length; i++) {
                ...
                if ((radio === "all") || (radio === "first" && i === 0) || (radio === "last" && i === (postedRegions.length - 1))) {
                    ...
                    newFlag.style = "padding: 0px 0px 0px 4px; vertical-align: -1px; display: inline-block; height: 11px; position: relative;";
